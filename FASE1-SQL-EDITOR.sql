-- Fase 1, DB-delen — kør dette i SQL-editoren (Lovable Cloud → backend, eller Supabase Studio)
-- hvis `supabase db push` ikke er mulig. Indholdet er migrationen
-- supabase/migrations/20260610130000_skat_engine.sql plus seed + virksomhedstype.
-- Når den er kørt: slet denne fil.

-- ============ 1. Migration (identisk med 20260610130000_skat_engine.sql) ============
...
CREATE TABLE IF NOT EXISTS public.accounts (
  ...
  vat_code TEXT NOT NULL DEFAULT 'NONE' CHECK (vat_code IN (
    'U25','UEUV','UEUY','UEKS','I25','IEUV','IEUY','IVKU','REP','NONE'
  )),
  tax_line TEXT,
  energy_levy TEXT,
...
DROP POLICY IF EXISTS "Company owners can manage accounts" ON public.accounts;
...
-- ============ 2. Seed kontoplan for alle eksisterende virksomheder ============

SELECT public.seed_default_kontoplan(id) FROM public.companies;

-- ============ 3. Din virksomhedstype og momsperiode ============

UPDATE public.companies
SET company_type = 'enkeltmandsvirksomhed', vat_period = 'halvaarlig';

-- ============ 4. Marker migrationen som kørt (kun hvis du senere vil bruge `supabase db push`) ============
-- INSERT INTO supabase_migrations.schema_migrations (version) VALUES ('20260610130000');
