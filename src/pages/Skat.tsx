...
import { Copy, Check, ChevronDown, ChevronUp, Building2, FileText } from "lucide-react";
...
export default function Skat() {
  ...
  // Will be populated from real data later
  const taxRubrikker: TaxRubrik[] = [];
  ...
  const fiscalYear = company ? new Date(company.fiscal_year_start).getFullYear() : "";
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {company?.name || "—"} {company?.cvr ? `· CVR ${company.cvr}` : ""} · Regnskabsår {fiscalYear}
          </p>
        </div>
        {taxRubrikker.length > 0 && (
